...
/*
  Benchmark code for single-threaded Spielman encoding with an extension field. 
...
int main(int argc, char* argv[])
{
  ...
  UnpackedExtensionField* array_reordered = new UnpackedExtensionField[2 * N / 8];
  ...
  for(uint32_t j = 0; j < rep; j++) {
    ...
    for (uint32_t r = 0; r < spielman.col_elements; r++) {
      for (uint32_t i = 0; i < spielman.k; i++) {
        array_reordered[i*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = UnpackedExtensionField::random();
        array_reordered[(i+spielman.k)*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = UnpackedExtensionField();
      }
    }
    ...
    for(size_t i = 0; i < 2 * N / 8 * (sizeof(UnpackedExtensionField) / sizeof(__m512i)); i++)

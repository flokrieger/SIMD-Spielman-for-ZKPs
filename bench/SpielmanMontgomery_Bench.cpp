...
/*
  Benchmark code for single-threaded Spielman encoding with a Montgomery field. 
...
int main(int argc, char* argv[])
{
  ...
  SpielmanCode<UnpackedMontgomeryPrimeField> spielman(N, seed, slices);
  ...
  for(uint32_t j = 0; j < rep; j++) {
    ...
    for (uint32_t r = 0; r < spielman.col_elements; r++) {
      for (uint32_t i = 0; i < spielman.k; i++) {
        array_reordered[i*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = UnpackedMontgomeryPrimeField::random();
        array_reordered[(i+spielman.k)*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = UnpackedMontgomeryPrimeField();
      }
    }

...
/*
  Testing code for multi-threaded Spielman encoding with an extension field. This expects
...
{  
  ...
  SpielmanCodeMT<ExtensionField> spielman_mt(N, seed, slices);
  ExtensionField* array_reordered = new ExtensionField[2*spielman_mt.k*column_size_AVX512];
  ...
  for (uint32_t r = 0; r < spielman_mt.col_elements; r++) {
    for (uint32_t i = 0; i < spielman_mt.k; i++) {
      ...
      uint64_t input_real[8];
      uint64_t input_img[8];
      for(int s = 0; s < 8; s++) {
        int cnt = fscanf(f_in, "%lx %lx\n", &input_real[s], &input_img[s]);
        assert(cnt == 2);
      }
      
      array_reordered[i*spielman_mt.col_elements + (r % (spielman_mt.col_elements)) + (r / (spielman_mt.col_elements)) * 2 * spielman_mt.k * spielman_mt.col_elements] = ExtensionField(_mm512_loadu_si512(input_real), _mm512_loadu_si512(input_img));
      array_reordered[(i+spielman_mt.k)*spielman_mt.col_elements + (r % (spielman_mt.col_elements)) + (r / (spielman_mt.col_elements)) * 2 * spielman_mt.k * spielman_mt.col_elements] = ExtensionField();
    }
  }
  ...
  for (uint32_t r = 0; r < spielman_mt.col_elements; r++) {
    for (uint32_t i = 0; i < spielman_mt.k * 2; i++) {
      uint64_t reference_real[8];
      uint64_t reference_img[8];
      ExtensionField_scalar reference[8];
      ExtensionField_scalar result[8];
        
      for(int s = 0; s < 8; s++) {
        int cnt = fscanf(f_ref, "%lx %lx\n", reference_real + s, reference_img + s);
        assert(cnt == 2);
        reference[s] = ExtensionField_scalar(reference_real[s], reference_img[s]);
      }
      ...
      for(uint32_t s = 0; s < 8; s++) {
        if(reference[s] != result[s]) {
          printf("Mismatch at row %u col %u (index %u): ref = %016lx_%016lx, res = %016lx_%016lx\n", 
                r*8+s, i, idx, reference_real[s], reference_img[s], result[s].real, result[s].img);
          error = 1;
        }
      }
    }

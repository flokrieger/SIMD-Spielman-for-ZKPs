...
/*
  Testing code for single-threaded Spielman encoding with a Montgomery field. This expects
...
int main()
{
  ...
  auto array_reordered = new UnpackedMontgomeryPrimeField[2*spielman.k*column_size_AVX512];
  ...
  for (uint32_t r = 0; r < spielman.col_elements; r++) {
    for (uint32_t i = 0; i < spielman.k; i++) {
      ...
      array_reordered[i*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = MontgomeryPrimeField(_mm512_loadu_si512(input_hi), _mm512_loadu_si512(input_lo));
      array_reordered[(i+spielman.k)*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = MontgomeryPrimeField();
    }
  }

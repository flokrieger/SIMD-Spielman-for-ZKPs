...
/*
  Testing code for multi-threaded Spielman encoding with a Mersenne field. This expects
...
{  
  ...
  char filename[256];
  snprintf(filename, sizeof(filename), "../tv/%s_mersenne_N%d_in.txt", BRAKEDOWN ? "bd_spielman" : "spielman", lgN);
  FILE* f_in = fopen(filename, "r");
  snprintf(filename, sizeof(filename), "../tv/%s_mersenne_N%d_out.txt", BRAKEDOWN ? "bd_spielman" : "spielman", lgN);
  ...
  SpielmanCodeMT<UnpackedMersennePrimeField> spielman_mt(N, seed, slices);
  UnpackedMersennePrimeField* array_reordered = new UnpackedMersennePrimeField[2*spielman_mt.k*column_size_AVX512];
  ...
  for (uint32_t r = 0; r < spielman_mt.col_elements; r++) {
    for (uint32_t i = 0; i < spielman_mt.k; i++) {
      ...
      array_reordered[i*spielman_mt.col_elements + (r % (spielman_mt.col_elements)) + (r / (spielman_mt.col_elements)) * 2 * spielman_mt.k * spielman_mt.col_elements] = MersennePrimeField(_mm512_loadu_si512(input_hi), _mm512_loadu_si512(input_lo));
      array_reordered[(i+spielman_mt.k)*spielman_mt.col_elements + (r % (spielman_mt.col_elements)) + (r / (spielman_mt.col_elements)) * 2 * spielman_mt.k * spielman_mt.col_elements] = MersennePrimeField();
    }
  }

...
/*
  Testing code for single-threaded verifier-side Spielman encoding with a Montgomery field. 
  This expects reference files in tv/ folder. 
*/
...
int main()
{
  ...
  auto array_reordered = new UnpackedMontgomeryPrimeFieldVerif[2*spielman.k];
  ...
  for (uint32_t r = 0; r < spielman.col_elements; r++) {
    for (uint32_t i = 0; i < spielman.k; i++) {
      ...
      array_reordered[i*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = MontgomeryPrimeFieldVerif(_mm_loadu_si128((__m128i*)input_hi), _mm_loadu_si128((__m128i*)input_lo));
      array_reordered[(i+spielman.k)*spielman.col_elements + (r % (spielman.col_elements)) + (r / (spielman.col_elements)) * 2 * spielman.k * spielman.col_elements] = MontgomeryPrimeFieldVerif();
    }
  }
      ...
      for(uint32_t s = 0; s < 2; s++) {
        ...
        if(reference[s] != result[s]) {
          printf("Mismatch at row %u col %u (index %u): ref = %016lx_%016lx, res = %016lx_%016lx\n", 
                r*2+s, i, idx, reference_hi[s], reference_lo[s], (uint64_t)(result[s]>>64), (uint64_t)result[s]);
          error = 1;
        }
      }
    }
